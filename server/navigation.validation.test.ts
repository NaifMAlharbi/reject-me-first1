...
describe("persistent navigation layout", () => {
  it("keeps the homepage top bar and includes language plus theme controls", () => {
    const source = readFileSync(committeePagesPath, "utf8");

    expect(source).toContain('min-h-screen text-foreground');
    expect(source).toContain('container flex items-center justify-between py-4');
  ...
  it("keeps a persistent flow header with theme and reset controls", () => {
    const source = readFileSync(flowShellPath, "utf8");

    expect(source).toContain('container max-w-5xl py-6');
    expect(source).toContain('toggleTheme');
    expect(source).toContain('tertiaryAction.onClick');
  });
});
