...
export function CommitteeFlowProvider({ children }: { children: ReactNode }) {
  ...
  const buildStartInput = useCallback(() => {
    const structuredForRequest = draft.inputMode === "structured" ? draft.structured : defaultStructuredInput;
    const freeTextForRequest = draft.inputMode === "free" ? draft.freeText : "";
    const combinedText = getCombinedText(freeTextForRequest, structuredForRequest, draft.transcriptText);

    return {
      language: normalizeLanguage(combinedText, draft.preferredLanguage),
      freeText: freeTextForRequest,
      structured: structuredForRequest,
